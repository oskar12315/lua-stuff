...
local function Make(c, p)
    ...
    if p then for k,v in pairs(p) do if k~="Parent" then pcall(function() i[k]=v end) end end
    if p.Parent then i.Parent=p.Parent end end
    return i
end

local function Tw(i,d,p,s)
    return TweenService:Create(i,TweenInfo.new(d,s or Enum.EasingStyle.Quad,Enum.EasingDirection.Out),p):Play()
end

local function TwR(i,d,p,s)
...
function Drags.Init(lib)
    if Drags.ok then return end; Drags.ok=true
...
function Library.new(name)
    ...
    table.insert(self.conn, UserInputService.InputBegan:Connect(function(inp,gpe)
        ...
        for _,m in ipairs(self.mods) do
            if m.bk and m.bk==inp.KeyCode and m.bk~=Enum.KeyCode.Unknown then
    ...
    -- HEIGHT UPDATE LOOP - this is the key fix
    -- instead of relying on AbsoluteContentSize listeners (which race),
    -- we poll every frame and update heights
...
function Library:MkSearch()
    ...
    -- dropdown on search layer, NOT parented to searchFrame
...
function Library:DoSearch()
    ...
    TwR(self.searchDrop,0.2,{Size=UDim2.new(0,280,0,#hits*26+6)},Enum.EasingStyle.Quart)
end
...
function Library:Pulse(mod)
    ...
    task.spawn(function()
        for i=1,3 do
            ...
            TwR(mod.box,0.12,{BackgroundTransparency=0.3})
            task.wait(0.2)
            TwR(mod.box,0.2,{BackgroundTransparency=1})
            task.wait(0.25)
        end
    end)
end
...
function Library:MkKBW()
    ...
    Make("UIStroke",{Color=Theme.CatBorder,Thickness=1,Transparency=0.4,Parent=self.kbw})
    Make("UIPadding",{PaddingLeft=UDim.new(0,10),PaddingRight=UDim.new(0,10),PaddingTop=UDim.new(0,6),PaddingBottom=UDim.new(0,6),Parent=self.kbw})
    ...
    Make("TextLabel",{
        ...
        Font=F.B, Text="KEYBINDS", TextColor3=Color3.fromRGB(200,200,215),
    ...
    Make("UIListLayout",{Parent=self.kbwList,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,1)})
    ...
    RunService.Heartbeat:Connect(function()
        ...
        for _,m in ipairs(self.mods) do
            if m.on and m.bk and m.bk~=Enum.KeyCode.Unknown then
                idx=idx+1
                local r=Make("Frame",{Parent=self.kbwList,BackgroundTransparency=1,Size=UDim2.new(1,0,0,16),LayoutOrder=idx})
                Make("TextLabel",{Parent=r,BackgroundTransparency=1,Size=UDim2.new(0.7,0,1,0),Font=F.R,Text=m.name,TextColor3=Theme.Accent,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left})
                Make("TextLabel",{Parent=r,BackgroundTransparency=1,Position=UDim2.new(0.7,0,0,0),Size=UDim2.new(0.3,0,1,0),Font=F.R,Text="["..m.bk.Name.."]",TextColor3=Theme.BindTxt,TextSize=9,TextXAlignment=Enum.TextXAlignment.Right})
            end
        end
    end)
end
...
function Library:UAcc(c)
    Theme.Accent=c; Theme.ModOn=c; Theme.TgOn=c
...
function Library:Save(n)
    ...
    for id,o in pairs(self.opts) do
        ...
        elseif o.Type=="Keybind" then e.v=o.Value~=Enum.KeyCode.Unknown and o.Value.Name or"Unknown"; e.m=o.Mode end
        d.opts[#d.opts+1]=e
    end
    for _,m in ipairs(self.mods) do
        d.mods[#d.mods+1]={id=m.fid,on=m.on,bk=m.bk and m.bk~=Enum.KeyCode.Unknown and m.bk.Name or nil,bm=m.bm}
    end
...
function Library:Load(n)
    ...
    if d.opts then for _,e in ipairs(d.opts) do
        ...
        if o and o.Set then pcall(function()
            ...
            elseif o.Type=="Keybind" then o:Set(Enum.KeyCode[e.v] or Enum.KeyCode.Unknown,e.m) end
        end) end
    end end
    if d.mods then for _,ms in ipairs(d.mods) do
        for _,m in ipairs(self.mods) do
            if m.fid==ms.id then
...
-- ═══════════════════════════════════
-- CATEGORY
-- The key insight: use AutomaticSize ONLY on the module list,
-- and just let the category frame also use AutomaticSize.
-- No manual height tracking needed. The previous approach
-- of manual height was racing with layout computation.
-- ═══════════════════════════════════
function Library:Cat(catName)
    ...
    -- THE FIX: Use a simple outer frame with AutomaticSize
    -- and an inner frame that we can clip for collapse
    cat.frame = Make("Frame", {
        ...
        -- NOT using AutomaticSize on this frame
        -- height managed by heartbeat loop
        LayoutOrder = idx,
        ZIndex = 1,
    })
    ...
    -- module list - ClipsDescendants for collapse animation
    cat.ml = Make("Frame", {
        Parent = cat.frame, BackgroundTransparency = 1,
    ...
    -- The heartbeat loop in Library.new handles height updates
    ...
    function cat:Collapse()
        self.collapsed = true
        arrow.Text = "▶"
        TwR(self.ml, 0.3, { Size = UDim2.new(1, -24, 0, 0) }, Enum.EasingStyle.Quart)
        TwR(self.frame, 0.3, { Size = UDim2.new(0, 200, 0, 36) }, Enum.EasingStyle.Quart)
        task.delay(0.3, function() if self.collapsed then sep.Visible = false end end)
    end
...
function Library._Mod(cat, name)
    ...
    mod.box = Make("Frame", {
        ...
        BorderSizePixel=0, Size=UDim2.new(1,0,0,26), LayoutOrder=mi, ClipsDescendants=true, ZIndex=2,
    })
    ...
    mod.ab = Make("Frame", {
        ...
        BorderSizePixel=0, Position=UDim2.new(0,0,0,2), Size=UDim2.new(0,2,1,-4), ZIndex=4,
    })
    ...
    local function recalc()
        task.defer(function()
            ...
            if mod.exp then
                ...
                TwR(mod.of,0.3,{Size=UDim2.new(1,-8,0,h)},Enum.EasingStyle.Quart)
                TwR(mod.box,0.3,{Size=UDim2.new(1,0,0,26+h)},Enum.EasingStyle.Quart)
            end
        end)
    end
    ...
    hb.MouseButton2Click:Connect(function()
        ...
        if mod.exp then
            ...
            lib:CPop()
            TwR(mod.of,0.25,{Size=UDim2.new(1,-8,0,0)},Enum.EasingStyle.Quart)
            TwR(mod.box,0.25,{Size=UDim2.new(1,0,0,26)},Enum.EasingStyle.Quart)
        end
    end)
    ...
    function mod:Toggle(tn,def,cb)
        local id=self.fid.."."..tn; local o={Type="Toggle",Value=def or false,Callback=cb,lb=tn}; self.oc=self.oc+1
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(1,-44,1,0),Font=F.R,Text=tn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local bg=Make("Frame",{Parent=row,BackgroundColor3=o.Value and Theme.TgOn or Theme.TgOff,BorderSizePixel=0,Position=UDim2.new(1,-36,0.5,-7),Size=UDim2.new(0,30,0,14),ZIndex=6})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=bg})
        if o.Value then lib:Trk(bg,"BackgroundColor3") end
        local kn=Make("Frame",{Parent=bg,BackgroundColor3=Theme.Knob,BorderSizePixel=0,Position=o.Value and UDim2.new(1,-13,0.5,-5) or UDim2.new(0,2,0.5,-5),Size=UDim2.new(0,10,0,10),ZIndex=7})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=kn})
        local btn=Make("TextButton",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),Text="",ZIndex=8})
        function o:Set(v) o.Value=v; Tw(bg,0.2,{BackgroundColor3=v and Theme.TgOn or Theme.TgOff}); Tw(kn,0.2,{Position=v and UDim2.new(1,-13,0.5,-5) or UDim2.new(0,2,0.5,-5)}); if o.Callback then pcall(o.Callback,v) end end
        btn.MouseButton1Click:Connect(function() o:Set(not o.Value) end)
        lib.opts[id]=o; self.ol[#self.ol+1]=o; task.defer(recalc); return o
    end

    -- SLIDER
    function mod:Slider(sn,def,mn,mx,cb,sfx,dec)
        sfx=sfx or""; dec=dec or 1; def=def or mn
        local id=self.fid.."."..sn; local o={Type="Slider",Value=def,Callback=cb,lb=sn}; self.oc=self.oc+1
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,34),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(0.6,0,0,15),Font=F.R,Text=sn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local vl=Make("TextLabel",{Parent=row,BackgroundTransparency=1,Position=UDim2.new(0.6,0,0,0),Size=UDim2.new(0.4,0,0,15),Font=F.S,Text=tostring(def)..sfx,TextColor3=Theme.OptVal,TextSize=12,TextXAlignment=Enum.TextXAlignment.Right,ZIndex=6})
        local tr=Make("Frame",{Parent=row,BackgroundColor3=Theme.SlBg,BorderSizePixel=0,Position=UDim2.new(0,0,0,19),Size=UDim2.new(1,0,0,5),ZIndex=6})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=tr})
        local p=math.clamp((def-mn)/(mx-mn),0,1)
        local fl=Make("Frame",{Parent=tr,BackgroundColor3=Theme.Accent,BorderSizePixel=0,Size=UDim2.new(p,0,1,0),ZIndex=7})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=fl}); lib:Trk(fl,"BackgroundColor3")
        local kb=Make("Frame",{Parent=tr,BackgroundColor3=Theme.Knob,BorderSizePixel=0,Position=UDim2.new(p,-5,0.5,-5),Size=UDim2.new(0,10,0,10),ZIndex=8})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=kb})
        local sb=Make("TextButton",{Parent=tr,BackgroundTransparency=1,Size=UDim2.new(1,0,1,14),Position=UDim2.new(0,0,0,-7),Text="",ZIndex=9})
        local sd={on=false}
        sd.fn=function(inp) local pp=math.clamp((inp.Position.X-tr.AbsolutePosition.X)/tr.AbsoluteSize.X,0,1); local val=math.floor((mn+(mx-mn)*pp)*(10^dec)+0.5)/(10^dec); o.Value=val; vl.Text=tostring(val)..sfx; fl.Size=UDim2.new(pp,0,1,0); kb.Position=UDim2.new(pp,-5,0.5,-5); if cb then cb(val) end end
        Drags.sl[#Drags.sl+1]=sd
        function o:Set(v) v=math.clamp(v,mn,mx); local pp=(v-mn)/(mx-mn); o.Value=v; vl.Text=tostring(v)..sfx; fl.Size=UDim2.new(pp,0,1,0); kb.Position=UDim2.new(pp,-5,0.5,-5); if cb then cb(v) end end
    ...
    function mod:Dropdown(dn,items,def,cb)
        local id=self.fid.."."..dn; local o={Type="Dropdown",Value=def or items[1],Callback=cb,lb=dn}; self.oc=self.oc+1
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(0.5,0,1,0),Font=F.R,Text=dn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local vb=Make("TextButton",{Parent=row,BackgroundTransparency=1,Position=UDim2.new(0.5,0,0,0),Size=UDim2.new(0.5,0,1,0),Font=F.S,Text=tostring(o.Value),TextColor3=Theme.OptVal,TextSize=12,TextXAlignment=Enum.TextXAlignment.Right,AutoButtonColor=false,ZIndex=6})
        local pf=Make("Frame",{Parent=lib.pLayer,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Size=UDim2.new(0,120,0,0),ClipsDescendants=true,Visible=false,ZIndex=110})
        Make("UICorner",{CornerRadius=UDim.new(0,4),Parent=pf}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=pf})
        Make("UIListLayout",{Parent=pf,SortOrder=Enum.SortOrder.LayoutOrder}); Make("UIPadding",{PaddingTop=UDim.new(0,2),PaddingBottom=UDim.new(0,2),Parent=pf})
        local ibs={}
        for i,item in ipairs(items) do
            local ib=Make("TextButton",{Parent=pf,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Size=UDim2.new(1,0,0,22),Font=F.R,Text=item,TextColor3=(item==o.Value) and Theme.Accent or Theme.OptText,TextSize=11,AutoButtonColor=false,LayoutOrder=i,ZIndex=111})
            ibs[#ibs+1]=ib
            ...
            ib.MouseButton1Click:Connect(function() o.Value=item; vb.Text=item; for _,b in ipairs(ibs) do b.TextColor3=(b.Text==item) and Theme.Accent or Theme.OptText end; lib:CPop(); if cb then cb(item) end end)
        end
        ...
        vb.MouseButton1Click:Connect(function()
            if lib.popup and lib.popup.fr==pf then lib:CPop(); return end
            local ap,as=vb.AbsolutePosition,vb.AbsoluteSize; local w=math.max(as.X,100)
            pf.Position=UDim2.new(0,ap.X+as.X-w,0,ap.Y+as.Y+2); pf.Size=UDim2.new(0,w,0,0); pf.Visible=true
            TwR(pf,0.2,{Size=UDim2.new(0,w,0,#items*22+4)},Enum.EasingStyle.Quart)
            lib:OPop({fr=pf,cl=function() TwR(pf,0.15,{Size=UDim2.new(0,w,0,0)},Enum.EasingStyle.Quart); task.delay(0.15,function() pf.Visible=false end) end})
        end)
        lib.opts[id]=o; self.ol[#self.ol+1]=o; task.defer(recalc); return o
    end
    ...
    function mod:ColorPicker(cn,def,cb)
        def=def or Color3.new(1,0,0); local h,s,v=HSV(def)
        local id=self.fid.."."..cn; local o={Type="ColorPicker",Value=def,Callback=cb,lb=cn}; self.oc=self.oc+1
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(1,-28,0,22),Font=F.R,Text=cn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local pv=Make("TextButton",{Parent=row,BackgroundColor3=def,BorderSizePixel=0,Position=UDim2.new(1,-20,0.5,-6),Size=UDim2.new(0,16,0,12),Text="",AutoButtonColor=false,ZIndex=6})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=pv}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=pv})
        local pnl=Make("Frame",{Parent=lib.pLayer,BackgroundColor3=Theme.PkBg,BorderSizePixel=0,Size=UDim2.new(0,180,0,100),Visible=false,ZIndex=110})
        Make("UICorner",{CornerRadius=UDim.new(0,5),Parent=pnl}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=pnl})
        local svB=Make("Frame",{Parent=pnl,BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Position=UDim2.new(0,8,0,8),Size=UDim2.new(1,-32,1,-16),ZIndex=111,ClipsDescendants=true})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=svB})
        local hO=Make("Frame",{Parent=svB,BackgroundColor3=Color3.fromHSV(h,1,1),Size=UDim2.new(1,0,1,0),ZIndex=112})
        Make("UIGradient",{Parent=hO,Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)})})
        local bO=Make("Frame",{Parent=svB,BackgroundColor3=Color3.new(0,0,0),Size=UDim2.new(1,0,1,0),ZIndex=113})
        Make("UIGradient",{Parent=bO,Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)}),Rotation=90})
        local svC=Make("Frame",{Parent=svB,BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Position=UDim2.new(s,-5,1-v,-5),Size=UDim2.new(0,10,0,10),ZIndex=116})
        Make("UICorner",{CornerRadius=UDim.new(1,0),Parent=svC}); Make("UIStroke",{Color=Color3.new(0,0,0),Thickness=1.5,Parent=svC})
        local svBt=Make("TextButton",{Parent=svB,BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),Text="",ZIndex=117})
        local hBr=Make("Frame",{Parent=pnl,BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Position=UDim2.new(1,-20,0,8),Size=UDim2.new(0,10,1,-16),ZIndex=111})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=hBr})
        Make("UIGradient",{Parent=hBr,Rotation=90,Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,0,0)),ColorSequenceKeypoint.new(0.167,Color3.fromRGB(255,255,0)),ColorSequenceKeypoint.new(0.333,Color3.fromRGB(0,255,0)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(0,255,255)),ColorSequenceKeypoint.new(0.667,Color3.fromRGB(0,0,255)),ColorSequenceKeypoint.new(0.833,Color3.fromRGB(255,0,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,0,0))})})
        local hC=Make("Frame",{Parent=hBr,BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Position=UDim2.new(0,-2,h,-2),Size=UDim2.new(1,4,0,4),ZIndex=116})
        Make("UICorner",{CornerRadius=UDim.new(0,2),Parent=hC}); Make("UIStroke",{Color=Color3.new(0,0,0),Thickness=1,Parent=hC})
        local hBt=Make("TextButton",{Parent=hBr,BackgroundTransparency=1,Size=UDim2.new(1,8,1,0),Position=UDim2.new(0,-4,0,0),Text="",ZIndex=117})
        local function upd() o.Value=Color3.fromHSV(math.clamp(h,0,0.999),s,v); pv.BackgroundColor3=o.Value; hO.BackgroundColor3=Color3.fromHSV(math.clamp(h,0,0.999),1,1); svC.Position=UDim2.new(s,-5,1-v,-5); hC.Position=UDim2.new(0,-2,h,-2); if cb then cb(o.Value) end end
        function o:Set(c) h,s,v=HSV(c); upd() end
        local svD={on=false}; svD.fn=function(inp) s=math.clamp((inp.Position.X-svB.AbsolutePosition.X)/svB.AbsoluteSize.X,0,1); v=1-math.clamp((inp.Position.Y-svB.AbsolutePosition.Y)/svB.AbsoluteSize.Y,0,1); upd() end; Drags.sv[#Drags.sv+1]=svD
        local hD={on=false}; hD.fn=function(inp) h=math.clamp((inp.Position.Y-hBr.AbsolutePosition.Y)/hBr.AbsoluteSize.Y,0,0.999); upd() end; Drags.hu[#Drags.hu+1]=hD
        ...
        pv.MouseButton1Click:Connect(function()
            if lib.popup and lib.popup.fr==pnl then lib:CPop(); return end
            local ap=pv.AbsolutePosition; pnl.Position=UDim2.new(0,ap.X-160,0,ap.Y+16); pnl.Visible=true
            lib:OPop({fr=pnl,cl=function() pnl.Visible=false; svD.on=false; hD.on=false end})
        end)
        lib.opts[id]=o; self.ol[#self.ol+1]=o; task.defer(recalc); return o
    end
    ...
    function mod:Keybind(kn,def,cb)
        ...
        local id=self.fid.."."..kn; local o={Type="Keybind",Value=def,Mode="toggle",Callback=cb,lb=kn}; self.oc=self.oc+1
        mod.bk=def; mod.bm="toggle"; mod:UB()
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,22),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(0.4,0,1,0),Font=F.R,Text=kn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local bb=Make("TextButton",{Parent=row,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Position=UDim2.new(1,-55,0.5,-9),Size=UDim2.new(0,52,0,18),Font=F.R,Text=def~=Enum.KeyCode.Unknown and def.Name or"none",TextColor3=Theme.OptVal,TextSize=10,AutoButtonColor=false,ZIndex=6})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=bb}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=bb})
        local mf=Make("Frame",{Parent=lib.pLayer,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Size=UDim2.new(0,80,0,0),ClipsDescendants=true,Visible=false,ZIndex=110})
        Make("UICorner",{CornerRadius=UDim.new(0,4),Parent=mf}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=mf})
        Make("UIListLayout",{Parent=mf,SortOrder=Enum.SortOrder.LayoutOrder}); Make("UIPadding",{PaddingTop=UDim.new(0,2),PaddingBottom=UDim.new(0,2),Parent=mf})
        local modes={"toggle","hold","always"}; local mbs={}
        for i,m in ipairs(modes) do
            local mb=Make("TextButton",{Parent=mf,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Size=UDim2.new(1,0,0,20),Font=F.R,Text=m,TextColor3=(m==o.Mode) and Theme.Accent or Theme.OptText,TextSize=11,AutoButtonColor=false,LayoutOrder=i,ZIndex=111})
            mbs[#mbs+1]=mb
            ...
            mb.MouseButton1Click:Connect(function() o.Mode=m; mod.bm=m; for _,b in ipairs(mbs) do b.TextColor3=(b.Text==m) and Theme.Accent or Theme.OptText end; lib:CPop(); if m=="always" then mod:SetOn(true) end end)
        end
        local ls={on=false}; ls.set=function(key) o.Value=key; mod.bk=key; bb.Text=key~=Enum.KeyCode.Unknown and key.Name or"none"; mod:UB(); Tw(bb,0.1,{TextColor3=Theme.OptVal}) end
        lib.kbListen[#lib.kbListen+1]=ls
        ...
        bb.MouseButton2Click:Connect(function()
            if lib.popup and lib.popup.fr==mf then lib:CPop(); return end
            ...
            TwR(mf,0.2,{Size=UDim2.new(0,80,0,#modes*20+4)},Enum.EasingStyle.Quart)
            lib:OPop({fr=mf,cl=function() TwR(mf,0.15,{Size=UDim2.new(0,80,0,0)},Enum.EasingStyle.Quart); task.delay(0.15,function() mf.Visible=false end) end})
        end)
        function o:Set(key,mode) o.Value=key; mod.bk=key; bb.Text=key~=Enum.KeyCode.Unknown and key.Name or"none"; if mode then o.Mode=mode; mod.bm=mode; for _,b in ipairs(mbs) do b.TextColor3=(b.Text==mode) and Theme.Accent or Theme.OptText end end; mod:UB() end
        lib.opts[id]=o; self.ol[#self.ol+1]=o; task.defer(recalc); return o
    end
    ...
    function mod:Button(text,cb)
        self.oc=self.oc+1
        local btn=Make("TextButton",{Parent=self.oi,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Size=UDim2.new(1,0,0,22),Font=F.S,Text=text,TextColor3=Theme.OptVal,TextSize=11,AutoButtonColor=false,LayoutOrder=self.oc,ZIndex=5})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=btn}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=btn})
    ...
    function mod:TextBox(tn,def,ph,cb)
        self.oc=self.oc+1; local o={Type="TextBox",Value=def or"",lb=tn}
        local row=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,36),LayoutOrder=self.oc,ZIndex=5})
        Make("TextLabel",{Parent=row,BackgroundTransparency=1,Size=UDim2.new(1,0,0,14),Font=F.R,Text=tn,TextColor3=Theme.OptText,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        local tb=Make("TextBox",{Parent=row,BackgroundColor3=Theme.DrBg,BorderSizePixel=0,Position=UDim2.new(0,0,0,16),Size=UDim2.new(1,0,0,18),Font=F.R,Text=def or"",PlaceholderText=ph or"",PlaceholderColor3=Theme.BindTxt,TextColor3=Theme.OptVal,TextSize=11,ClearTextOnFocus=false,ZIndex=6})
        Make("UICorner",{CornerRadius=UDim.new(0,3),Parent=tb}); Make("UIStroke",{Color=Theme.DrBor,Thickness=1,Parent=tb})
        Make("UIPadding",{PaddingLeft=UDim.new(0,5),PaddingRight=UDim.new(0,5),Parent=tb})
        ...
        lib.opts[self.fid.."."..tn]=o; self.ol[#self.ol+1]=o; task.defer(recalc); return o
    end

    -- LABEL
    function mod:Label(text)
        self.oc=self.oc+1
        Make("TextLabel",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,16),Font=F.R,Text=text,TextColor3=Theme.BindTxt,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=self.oc,ZIndex=5})
        task.defer(recalc)
    end
    ...
    function mod:Separator()
        self.oc=self.oc+1
        local s=Make("Frame",{Parent=self.oi,BackgroundTransparency=1,Size=UDim2.new(1,0,0,6),LayoutOrder=self.oc,ZIndex=5})
        Make("Frame",{Parent=s,BackgroundColor3=Theme.Sep,BorderSizePixel=0,Position=UDim2.new(0,0,0.5,0),Size=UDim2.new(1,0,0,1),ZIndex=6})
        task.defer(recalc)
    end
